...
Rmarkdown is using `Markdown syntax`. This is relatively simple syntax to use to create a document with images, tables, links, headers, footers etc that is reproducible and also easy to read. I've even written a manuscript in Rmarkdown and presentations but you can also write your thesis, a book, website - and the list goes on! 

## Example workflow to include Rmarkdown

Here is an example workflow of how it may work for you:
...
It is also simple to have analysis and graphics code within your Rmarkdown report, which means everything for your report is in one document.
...
By using Rmarkdown, once a report is "written" the first time, you then **knit** it to create the output file (html, word, pdf). So if the data is updated, as you have "coded" your report, you only need to reknit your report. Magic!
...
Rather than reproducing the wheel, [checkout this cheatsheet](https://github.com/rstudio/cheatsheets/raw/master/rmarkdown-2.0.pdf) to look at formatting options in Rmarkdown (headings, bold text, italics, lists etc etc). 
